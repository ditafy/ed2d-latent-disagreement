...
from config import PRINT_DEBATE_LOGS


def _log(msg: str):
    if PRINT_DEBATE_LOGS:
        print(msg)
...
class EvidenceSystem:
    ...
    def extract_keywords(self, news_text: str) -> List[str]:
        ...
        try:
            ...
            _log(f"[⚠️ Keyword extraction failed] {e}")
            return []
    ...
    def search_wikipedia(self, keyword: str) -> Optional[Dict]:
        ...
        try:
            ...
            _log(f"[⚠️ Wikipedia search failed for '{keyword}'] {e}")
            return None
    ...
    def evaluate_evidence_stance(self, news_text: str, evidence_info: Dict) -> str:
        ...
        try:
            ...
            _log(f"[⚠️ Evidence evaluation failed] {e}")
            return 'NEUTRAL'
    ...
    def gather_evidence(self, news_text: str) -> Dict[str, any]:
        ...
        for keyword in keywords:
            _log(f"🔍 Searching Wikipedia for: {keyword}")
            ...
            if wiki_result:
                ...
                _log(f"✅ Found: {wiki_result['title']} (Stance: {stance})")
            else:
                _log(f"❌ No Wikipedia entry found for: {keyword}")
