...
def _init_openai_client():
    """Create an OpenAI client that works for both legacy and v1 SDKs."""
...
class Agent:
    ...
    def _make_openai_request(self, messages: list, max_tokens: int, temperature: float) -> str:
        ...
        if hasattr(_openai_client, "chat") and hasattr(_openai_client.chat, "completions"):
        ...
        elif hasattr(_openai_client, "ChatCompletion"):
            ...
            raise RuntimeError("OpenAI client is not configured for chat completions.")
        try:
    ...
    def _calculate_max_tokens(self, messages: list) -> int:
        ...
        if available_tokens <= 0:
            raise ContextLengthExceeded(
                f"Context tokens {ctx_tokens} exceed limit {max_context} for model {self.model_name}"
            )
